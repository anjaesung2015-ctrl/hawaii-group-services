...
| 로그인 | **매니저/코치만 사용**. 매니저/총매니저/코치=이름 선택(PIN/비번 없음), 사장님(대표)=비밀번호. 일반직원 제외 |
...
- 이름 목록은 기존 `staff` 중 `is_active=1 AND role IN ('매니저','총매니저','코치')` 재활용. 일반직원·대표는 목록에서 제외.
...
- 상단: 매니저/코치 이름 드롭다운 선택 → 본인 보고. 사장님은 🔒 클릭 후 비번 → 전체 인원 드롭다운 노출, 골라서 조회.
